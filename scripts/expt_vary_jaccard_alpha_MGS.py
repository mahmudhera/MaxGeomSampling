...
We repeat this for different alpha values of alpha-MGS (alpha-MaxGeomSampling).
'''
...
if __name__ == '__main__':
    ...
    for _ in tqdm(range(num_trials)):
        ...
        for alpha in alpha_values_tested:
            sample_A = AlphaMaxGeomSample(alpha=alpha, seed=random_seed)
            sample_A.add_many_items(set_A)
            sample_B = AlphaMaxGeomSample(alpha=alpha, seed=random_seed)
            sample_B.add_many_items(set_B)
            estimated_jaccard = sample_A.jaccard_index(sample_B)
            all_recorded_results.append((alpha, true_jaccard, estimated_jaccard))

    # print results to a file
    with open(output_file, 'w') as f:
        f.write('alpha\tTrue_Jaccard\tEstimated_Jaccard\n')
