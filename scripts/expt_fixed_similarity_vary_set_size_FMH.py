...
def estimate_with_fmh(A, B, scale_factor, seeds, metric="jaccard"):
    ests = []
    a, b = len(A), len(B)
    sample_sizes_A = []
    sample_sizes_B = []
    ...
    for s in tqdm(seeds):
        m1 = FracMinHashSketch(scale=scale_factor, seed=s)
        m2 = FracMinHashSketch(scale=scale_factor, seed=s)
        m1.add_many_items(A)
        m2.add_many_items(B)
...
def run_experiment(
    ...
    with open(output_file, "w") as f:
        f.write("metric\tFMH_scale_factor\tstep\t|A|\t|B|\tmean_sample_size_A\tmean_sample_size_B\ttrue_sim\tmean_est\tmse\n")
    ...
    for step in tqdm(range(steps), desc="Steps"):
        ...
        ests, sample_sizes_A, sample_sizes_B = estimate_with_fmh(A, B, s, seeds, metric)
        ...
        with open(output_file, "a") as f:
            f.write(f"{metric}\t{s}\t{step}\t{len(A)}\t{len(B)}\t{mean_sample_size_A:.6f}\t{mean_sample_size_B:.6f}\t{true_sim:.6f}\t{mean_est:.6f}\t{err:.6e}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--s", type=float, default=0.001, help="FMH parameter s (scale factor).")
