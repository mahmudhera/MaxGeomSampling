...
if __name__ == "__main__":
    ...
    # from the df, only keep 30% of the rows for faster plotting
    ...
    plotting_alphas = [1, 0.85, 0.7, 0.45]
    ...
    for k in k_values_tested:
        subset = df[df['k'] == k]
        alpha = plotting_alphas[color_idx]
        plt.scatter(subset['True_Jaccard'], subset['Estimated_Jaccard'], 
                    label=f'MaxGeomHash ($b$={k})', color=colors[color_idx], 
                    alpha=alpha, marker=markers[color_idx], s=markersize)
        color_idx += 1
